<?php
  ...
  <div class="list-group list-group-flush" style="max-height: 500px; overflow-y: auto; padding: 10%;">
    ...
    foreach ($tiendas as $tienda) {
        ...
        // Obtener el estado del componente actual desde $_GET
        $posComponente = isset($_GET['posComponente']) ? $_GET['posComponente'] : 'activos';

        // Verificar si la tienda actual está seleccionada
        $isSelected = isset($_GET['idTienda']) && $_GET['idTienda'] == $tienda['id'];

        // Aplicar un estilo de fondo diferente si la tienda está seleccionada
        $backgroundStyle = $isSelected ? 'active' : '';

        echo '<strong>';
        echo '<a href="filtros.php?posComponente=' . urlencode($posComponente) . '&idTienda=' . $tienda['id'] . '" class="list-group-item list-group-item-action bg-white text-black tienda-link '.$backgroundStyle.'" style="font-family: Arimo, sans-serif; > <i class="fa-solid fa-store"></i> ' . $tienda['nombre'] . '<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-' . ($num_st == 0 ? 'light' : 'danger') . '">' . $num_st . '<span class="visually-hidden">unread messages</span></span> <p  class="text-grey" style="font-size: smaller; color: #888;">' . $tienda['nombre_region'] . '</p></a>';
        echo '</strong>';  
  ...
  if(!idTienda){
    ...
    cargarContenedor(idTienda, posComponente || 'activos');
  }
